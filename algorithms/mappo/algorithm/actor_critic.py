...
def get_shape_from_obs_space(obs_space):
    """Get observation shape from gym space."""
    if isinstance(obs_space, spaces.Box):
        ...
        return {key: get_shape_from_obs_space(subspace) 
...
class Actor(nn.Module):
    ...
    def forward(self, obs, rnn_states, masks, available_actions=None, deterministic=False):
        ...
        if rnn_states is None:
            rnn_states = torch.zeros((obs.shape[0], self._recurrent_N, self.hidden_size), **self.tpdv)
        else:
...
class Critic(nn.Module):
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        if rnn_states is None:
            # Khởi tạo zero tensor nếu chưa có RNN state
            rnn_states = torch.zeros((cent_obs.shape[0], self._recurrent_N, self.hidden_size), **self.tpdv)
        else:
            rnn_states = check(rnn_states).to(**self.tpdv)
        masks = check(masks).to(**self.tpdv)
