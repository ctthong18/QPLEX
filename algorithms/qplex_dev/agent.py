...
class QPLEXAgent:
    ...
    def select_action(self, obs: np.ndarray, state: np.ndarray, 
        ...
        """
        Select actions for all agents - FIXED VERSION.
        """
        with torch.no_grad():
            # Convert to tensor với đúng shape
            ...
            # Select actions - FIXED LOGIC
            if evaluate or random.random() > self.epsilon:
                # Greedy action selection
                if self.action_dim == 1:
                    # Discrete action space
                    actions = q_values.argmax(dim=-1).squeeze(0).cpu().numpy()  # (n_agents,)
                else:
                    # Continuous action space - use mean or sample from distribution
                    actions = q_values.squeeze(0).cpu().numpy()  # (n_agents, action_dim)
            else:
                # Random exploration
                if self.action_dim == 1:
                    # Discrete random actions
                    actions = np.random.randint(0, self.action_dim, size=(self.n_agents,))
                else:
                    # Continuous random actions
